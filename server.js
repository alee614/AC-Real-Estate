...
const con = mysql.createConnection({
	...
	host: "animal-crossing.cod4wcdciy33.us-west-2.rds.amazonaws.com",
	user: "admin",
	password: "testingdatabase",
	port: 3306,
	// has to be the name of the schema because the database can have the name of multiple schemas
	database: "animal_crossing",
...
// GET request for specific villager
...
// GET request to get userid based on email
...
// POST request to add a villager to user_villager table
...
// GET request of all animals associated with user
// will be working with getUserId
// editted so that this will pull from ANY lists -- must be inputted in the body what list
async function currentVillagers(req, res){
	const list = req.body.list;
	const user_id = req.body.user_id;
	console.log(user_id);
	// prints all the values of villager_id based on user_id
	await con.query(`SELECT villager_id FROM animal_crossing.${list} WHERE user_id = "${user_id}"`, (err, result, fields) => {
		if (err) throw err;
		res.json(result);
	})

}
app.get("/getcurrentvillagers", jsonParser, currentVillagers);
